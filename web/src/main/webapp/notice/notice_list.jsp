...
/*
페이지 생성방법
1. 한 페이지당 몇개씩 데이터를 출력할 것인지를 설정
2. 데이터베이스에 있는 데이터의 총 갯수 /한페이지당 갯수 (소수점)
3. Math.ceil 사용하는 이유는 1.1 1.6 => 올림으로 페이지가 추가 되도록 ㅏㅂ니다.
*/

//한 페이지당 10개씩 => 11 11/10 => 1.3xxx => 2 (ceil사용)
...
//총 데이터 갯수 -(페이지번호-1) * 한페이지당 출력 갯수)
	int total = Integer.parseInt(total_page) -((Integer.parseInt(pno)-1)*3); //총 개수//리스트 출력 번호를 출력하기위함
	for(f=0; f<notice.size(); f++){
%>
	<tr height="30" align="center">
		<td><%=total %></td> <!-- notice.get(f).get(0) -->
